...
- Student: stabilityai/stable-diffusion-3.5-medium (2.5B params, trainable, bfloat16)

Both share the same latent space. Projector g_phi is trained end-to-end with L_KL.

Training objective:
  L = L_distill + beta_kl * L_KL + beta_ent * L_entropy

  L_distill: energy-weighted MSE between teacher and student noise predictions.
             Gibbs weights q = softmax(-E/tau) up-weight high-quality teacher samples.
  L_KL:      DINO-style KL divergence between projected teacher hidden states and
             DINOv2 CLS token — KL(p_tea ‖ p_stu) where p = softmax(z / T).
             Sole training signal for the projector g_phi; student receives no
             gradient from this term.
  L_entropy: optional negative-entropy regulariser on q (beta_ent=0 by default).

NOTE on alignment architecture: h_T is captured from the *teacher* transformer's last
block (not the student's), so L_KL only directly trains the projector g_phi. The
...
def main():
    ...
    parser.add_argument("--beta_kl", type=float, default=0.5,
                        help="Weight for L_KL (DINO-style KL divergence between projected "
                             "teacher hidden states and DINOv2 CLS token). This is the sole "
                             "projector training signal.")
    parser.add_argument("--kl_temp", type=float, default=0.1,
                        help="Temperature for the KL divergence softmax. Lower = sharper "
                             "distributions, stronger gradient signal on dominant dimensions. "
                             "DINO uses 0.1 for student, 0.04 for teacher (we use symmetric).")
    ...
    parser.add_argument("--wandb_run_name", default=None, help="W&B run name (default: auto-generated by W&B).")
    ...
    while global_step < args.num_steps:
        ...
        metrics = {
            ...
            "train/mse_ref_mean": mse_ref_mean.item(),
        ...
        if global_step % args.log_every == 0:
            print(
                f"Step {global_step} | loss={metrics['train/loss']:.3f} | "
                f"distill={metrics['train/L_distill']:.3f} | "
                ...
                f"mse={metrics['train/mse_per_mean']:.3f}+/-{metrics['train/mse_per_std']:.3f} | "
                ...
                f"tau={metrics['train/tau']:.3f} T_kl={metrics['train/kl_temp']:.3f} | "
