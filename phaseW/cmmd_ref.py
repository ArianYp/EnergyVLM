...
The project's earlier `exp0/fidelity_cmmd.py` differs on three counts (ViT-L/14 @ 224, the HF
processor's resize-then-crop, and NO unit normalisation); its output is an unnormalised
CLIP-L/14 RBF-MMD, internally consistent but not comparable to published CMMD values.
"""
