...
class RGBReward:
    ...
    photograph: the exact reward of the projector-free reference experiment. Same preprocessing as
    the offline scorer (resize shortest edge 256 bicubic, centre crop 224, ImageNet stats) but on
    tensors, so gradients flow through the frozen VAE decoder and the frozen DINO into the latent.
    No uint8 / PIL round trip; the offline PIL path (OnlineScorer) stays the independent monitor."""

    def __init__(self, vae, dino, device, resize="bicubic_aa", dino_bf16=False):
        self.vae, self.dino = vae, dino
        self.resize, self.dino_bf16 = resize, dino_bf16
    ...
    def embed(self, lat):
        ...
        x = ((img + 1) / 2).clamp(0, 1)
        if self.resize == "bicubic_aa":
            x = F.interpolate(x, size=(256, 256), mode="bicubic", align_corners=False, antialias=True)
        else:                                                                      # ablation: plain bilinear, no antialias
            x = F.interpolate(x, size=(256, 256), mode="bilinear", align_corners=False, antialias=False)
        x = x[:, :, 16:240, 16:240]
        x = (x - self.mean) / self.std
        if self.dino_bf16:                                                         # ablation: DINO matmuls in bf16
            with torch.autocast("cuda", torch.bfloat16):
                h = self.dino(pixel_values=x).last_hidden_state
        else:
...
def pick_index(rec, variant, rng, alpha, temp, visit=0, sched_mode=None, support_m=1,
    ...
    if int((w > 0).sum()) == 1:            # deterministic arms: B2, B4, B5_*, W_energy_hard
        return int(w.argmax())
    if variant.endswith("_catfreeze"):
        # Frozen-Boltzmann (S4e): ONE draw per caption from the raw-score softmax, made by a
        # per-caption generator so it is the same on every visit and on every rank. Keeps the
        # Cat-1 selection distribution, removes per-visit resampling. The map changes with
        # --assign_seed, so seeds can vary the map or share it.
...
def selection_weights(rec, variant, alpha, temp):
    ...
    key = {"energy": "energy", "oracle": "endpoint_vqa", "dino": "dino_cos",
           ...
           "drift": "teacher_drift",
           # latent = cos( projector(z_K), DINO-patch(reference) ): the decode-free latent scorer
           # (phaseW/latent_scorer), distilled from dinop on held-out captions
           "latent": "latent_cos"}.get(variant.split("_")[1])
    ...
    if variant.endswith("_hard"):
        ...
        w[int(s.argmax())] = 1.0
        return w
    if variant.endswith(("_cat", "_full", "_catfreeze", "_mc")):
...
def main():
    ...
    ap.add_argument("--variant", required=True,
                    choices=["B2", "B4", "B5_latent", "B5_pixel",
                             ...
                             "CD_dinop_full",
                             # Frozen-Boltzmann: one softmax(S/T) draw per caption, fixed for the run
                             "CD_dinop_catfreeze",
                             # same-caption Monte-Carlo: --mc_draws iid draws from softmax(S/T) per
                             # visit, losses weighted by count/M, one clipped step (Full-N path)
                             "CD_dinop_mc",
                             # decode-free latent scorer (cache field latent_cos): argmax and exact
                             # Boltzmann weighting, for comparison with the RGB DINO-patch arms
                             "CD_latent_hard", "CD_latent_full"])
    ap.add_argument("--mc_draws", type=int, default=4, help="draws per visit for the _mc variant")
    ...
    ap.add_argument("--reward_proj", default=None, help="latent projector checkpoint; enables the reward term (reward_mode proj)")
    ap.add_argument("--reward_mode", default="proj", choices=["proj", "rgb"],
                    help="proj: reward through the latent projector; rgb: the exact reward, decode + DINO with gradients (no projector)")
    ...
    ap.add_argument("--reward_states_pick", default="least_noisy", choices=["least_noisy", "noisiest"], help="which --reward_states supervised inputs carry the reward")
    ap.add_argument("--reward_resize", default="bicubic_aa", choices=["bicubic_aa", "bilinear"], help="differentiable resize of the decoded prediction (rgb reward)")
    ap.add_argument("--reward_dino_dtype", default="fp32", choices=["fp32", "bf16"], help="precision of the DINO pass inside the rgb reward")
    ...
    is_full = args.variant.endswith(("_full", "_mc"))
    if is_full:
    ...
    # S4i: the reward machinery is LOADED whenever a reward mode is set (also at lambda = 0, the
    # zero-weight control) and the global torch RNG state is restored afterwards, so loading the
    # scorer / projector no longer changes the caption order relative to a reward-free run.
    reward_load = (args.reward_proj is not None or args.reward_mode == "rgb")
    reward_on = reward_load and args.reward_lambda != 0.0
    loss_cd_v = None
    proj = rgb_reward = None
    ...
    if reward_load:
        ...
        if scorer is None and (args.reward_mode == "rgb" or args.reward_refresh_every > 0 or args.reward_monitor_every > 0):
            scorer = OnlineScorer(pipe.vae, args.score_model, args.height, device)
        if args.reward_mode == "proj":
            ...
            rgb_reward = RGBReward(pipe.vae, scorer.dino, device, resize=args.reward_resize, dino_bf16=(args.reward_dino_dtype == "bf16"))
            pipe.vae.enable_gradient_checkpointing()                     # decoder activations at 512^2
            # verify the differentiable path against the PIL / uint8 offline scorer on teacher latents
            ...
            with torch.no_grad():
                ...
                _pil = torch.cat([scorer.score(_zc[i:i + 1], None) if False else (scorer.embed([scorer.Image.fromarray(x.permute(1, 2, 0).cpu().numpy()) for x in ((vae_decode(pipe.vae, _zc[i:i + 1]) + 1) / 2 * 255).round().clamp(0, 255).to(torch.uint8)]) * _er[i:i + 1]).sum(-1) for i in range(len(_zc))])
        ...
        if is_main:
            print(f"[reward] mode={args.reward_mode} projector {args.reward_proj} lambda={args.reward_lambda} states={args.reward_states} "
    ...
    while gstep < args.num_steps:
        ...
        if is_consistency:
            ...
            if is_full:
                ...
                if args.variant.endswith("_mc"):
                    # same-caption MC-M: M iid draws from pi this visit, weights = count / M. Same
                    # expected gradient as Cat-1 and Full-N, candidate-sampling variance / M, one
                    # clipped optimizer step; repeated draws reuse the trajectory via the weight.
                    _cnt = np.bincount(sel_rng.choice(len(w_full), size=args.mc_draws, p=w_full),
                                       minlength=len(w_full))
                    w_full = _cnt / float(args.mc_draws)
                ...
                if args.student_cfg > 0 and args.cfg_mode == "branch":
                    ...
                    if reward_on:
                        ...
                        _order = sorted(range(n_w), key=lambda i: float(sig_stu[i]), reverse=(args.reward_states_pick == "noisiest"))[:args.reward_states]
                        _xr = x_hat[_order]
