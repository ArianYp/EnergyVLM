...
class MMDiT(nn.Module):
    ...
    def __init__(
        ...
        use_orca: bool = False,
        orca_rank: int = 4,
        orca_ortho_method: str = "qr",
        orca_normalize_delta: bool = False,
        orca_stop_grad_gap: bool = False,
        orca_normalize_gap: bool = False,
        orca_clamp_gap: bool = False,
        orca_ddl_style: bool = False,
        orca_v2: bool = False,
        orca_loss_only: bool = False,
        orca_monitor_only: bool = False,
        orca_loss_mode: str = "none",
        orca_new_formulation: bool = False,
        orca_normalize_projections: bool = False,
        orca_gate_type: str = "mlp",
        orca_gate_threshold: float = 0.0,
        orca_eta: float = 0.1,
    ):
        super().__init__()
        ...
        if self.use_orca:
            ...
            self.orca_module = ORCA_T2I(
                ...
                d_dinov2=1024,
                rank=orca_rank,
                ortho_method=orca_ortho_method,
                normalize_delta=orca_normalize_delta,
                stop_grad_gap=orca_stop_grad_gap,
                normalize_gap=orca_normalize_gap,
                clamp_gap=orca_clamp_gap,
                ddl_style=orca_ddl_style,
                orca_v2=orca_v2,
                orca_loss_only=orca_loss_only,
                orca_monitor_only=orca_monitor_only,
                orca_loss_mode=orca_loss_mode,
                orca_new_formulation=orca_new_formulation,
                orca_normalize_projections=orca_normalize_projections,
                gate_threshold=orca_gate_threshold,
                gate_type=orca_gate_type,
                eta=orca_eta,
            )
            print(f"  Initialized ORCA T2I: rank={orca_rank}, ortho_method={orca_ortho_method}, normalize_delta={orca_normalize_delta}, stop_grad_gap={orca_stop_grad_gap}, normalize_gap={orca_normalize_gap}, clamp_gap={orca_clamp_gap}, ddl_style={orca_ddl_style}, orca_v2={orca_v2}, orca_loss_only={orca_loss_only}, orca_monitor_only={orca_monitor_only}, orca_loss_mode={orca_loss_mode}, orca_new_formulation={orca_new_formulation}, orca_normalize_projections={orca_normalize_projections}, gate_type={orca_gate_type}, layer={encoder_depth}, d_model={hidden_size}")
    ...
    def forward_core_with_concat(
            ...
            t5_emb: Optional[torch.Tensor] = None,
            dinov2_emb: Optional[torch.Tensor] = None,
            return_orca_diagnostics: Optional[bool] = False) -> torch.Tensor:
        if self.register_length > 0:
        ...
        # x is B, L, D
        B, L, D = x.shape
        zs = []  # Initialize empty (for vanilla/ORCA without projectors)
        orca_diagnostics = None  # Initialize for potential ORCA diagnostics
        for i, block in enumerate(self.joint_blocks):
            context, x = block(context, x, c=c_mod)

            if (i + 1) == self.encoder_depth:
                # ORCA correction (TRAINING ONLY, before REPA)
                if (self.use_orca
                    and self.training
                    and clip_text_emb is not None
                    and t5_emb is not None
                    and dinov2_emb is not None):
                    # Always return diagnostics if in loss-only or monitor-only mode or using loss_mode
                    if (return_orca_diagnostics or self.orca_module.orca_loss_only or
                        self.orca_module.orca_monitor_only or self.orca_module.orca_loss_mode != "none"):
                        x, orca_diagnostics = self.orca_module(
                            h_t=x,
                ...
                # REPA projection (after ORCA if enabled)
                if detach:
                    x_ = x.clone().detach()
        ...
        if return_orca_diagnostics and orca_diagnostics is not None:
            return x, zs, orca_diagnostics
        return x, zs
    ...
    def forward(
        ...
        x: (N, C, H, W) tensor of spatial inputs (latent representations)
        t: (N,) tensor of diffusion timesteps
        y: (N,) tensor of class labels (optional, not used for T2I)
        context: (N, L, 768) CLIP text context (for standard T2I)
        clip_text_emb: (N, 768) CLIP text pooled (for ORCA)
        t5_emb: (N, 2048) T5-XL pooled (for ORCA)
        dinov2_emb: (N, 256, 1024) DINOv2 patch tokens (for ORCA)
        """
        hw = x.shape[-2:]
        ...
        result = self.forward_core_with_concat(
            x, c, context, detach,
            clip_text_emb=clip_text_emb,
            t5_emb=t5_emb,
            dinov2_emb=dinov2_emb,
            return_orca_diagnostics=return_orca_diagnostics,
        )

        if return_orca_diagnostics and len(result) == 3:
            x, zs, orca_diagnostics = result
