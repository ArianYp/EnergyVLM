...
def create_npz_from_sample_folder(sample_dir, num=50_000):
    ...
    Handles non-sequential file numbering from multi-GPU generation.
    """
    import glob

    # Get all PNG files in the directory
    png_files = sorted(glob.glob(f"{sample_dir}/*.png"))
    print(f"Found {len(png_files)} PNG files in {sample_dir}")

    if len(png_files) == 0:
        raise ValueError(f"No PNG files found in {sample_dir}")

    # Take only the first 'num' samples
    png_files = png_files[:num]
    if len(png_files) < num:
        print(f"Warning: Only found {len(png_files)} files, requested {num}")
        num = len(png_files)
...
def main(args):
    ...
    model = MMDiT(
        ...
        use_orca=args.use_orca,  # Add use_orca flag for ORCA checkpoint compatibility
    ).to(device)

    # Setup data:
    all_dataset = MSCOCO256Features(path=args.data_dir, mode='val')  # Fixed: use --data-dir, removed ret_caption=True
    ...
    # Filter out ORCA module keys (not needed during inference)
    original_keys = len(state_dict)
    state_dict = {k: v for k, v in state_dict.items() if 'orca' not in k.lower()}
    filtered_keys = original_keys - len(state_dict)
    if filtered_keys > 0:
        print(f"Filtered out {filtered_keys} ORCA module keys (not needed for inference)")

    missing, unexpected = model.load_state_dict(state_dict, strict=False)
    if missing:
        print(f"Missing keys: {len(missing)} (expected if loading ORCA checkpoint)")
    if unexpected:
    ...
    total_samples = args.num_samples  # Fixed: use --num-samples argument
    if accelerator.is_main_process:
        print(f"Total number of images that will be sampled: {total_samples}")
        print(f"Model Parameters: {sum(p.numel() for p in model.parameters()):,}")
        if hasattr(model, 'projectors'):  # Fixed: check if projectors exist (REPA has them, vanilla/ORCA don't)
    ...
    total = 0
    # ClipSimilarity not needed - we only need generated images for FID
    # clipsim_sum = 0.
    # from utils import ClipSimilarity
    # clipsim_fn = ClipSimilarity(device=device)

    # Create iterator and limit to required iterations
    dataloader_iter = iter(val_dataloader)

    for iteration_idx in range(iterations):
        try:
            raw_image, _, context, raw_captions = next(dataloader_iter)
        except StopIteration:
            # If we run out of validation data, cycle back to beginning
            dataloader_iter = iter(val_dataloader)
            raw_image, _, context, raw_captions = next(dataloader_iter)
        ...
        # Log progress
        if accelerator.is_main_process:
            print(f"Generated {total}/{args.num_samples} samples")
            pbar.update(1)
    ...
    dist.barrier()
    if accelerator.is_main_process:
        create_npz_from_sample_folder(sample_folder_dir, args.num_samples)  # Fixed: use --num-samples
        # create_npz_from_sample_folder(real_sample_folder_dir, args.num_samples)
        print("Done.")
    dist.barrier()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--num-samples", type=int, default=30000,
                       help="Total number of samples to generate (default: 30000)")
    ...
    # ORCA
    parser.add_argument("--use-orca", action=argparse.BooleanOptionalAction, default=False,
                       help="Use ORCA model architecture (needed for ORCA checkpoint loading)")
