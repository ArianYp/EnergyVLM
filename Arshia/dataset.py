...
class MSCOCOFeatureDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # Training loop expects 4-tuple: (raw_image, vae_moments, clip_context, raw_captions)
        # raw_captions not used during training, only during evaluation
        raw_captions = ""  # Placeholder - not saved in preprocessed data

        return x, z, c, raw_captions
...
class CFGDataset(Dataset):  # for classifier free guidance
    ...
    def __getitem__(self, item):
        x, z, y, raw_captions = self.dataset[item]
        if random.random() < self.p_uncond:
            y = self.empty_token
        return x, z, y, raw_captions
...
class MSCOCO256Features(DatasetFactory):  # the moments calculated by Stable Diffusion image encoder & the contexts calculated by clip
    def __init__(self, path, cfg=True, p_uncond=0.1, mode='train'):
        ...
        if mode == 'val':
            ...
            print(f'Loaded {len(self.test)} validation samples')
            # assert len(self.test) == 40504  # Removed: hardcoded for COCO 2014, but data may be COCO 2017
            ...
            # assert len(self.train) == 82783  # Removed: hardcoded for COCO 2014, but data may be COCO 2017
            self.empty_context = np.load(os.path.join(path, 'empty_context.npy'))
