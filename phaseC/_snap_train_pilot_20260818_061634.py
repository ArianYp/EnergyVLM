...
def selection_weights(rec, variant, alpha, temp):
    ...
    key = {"energy": "energy", "oracle": "endpoint_vqa", "dino": "dino_cos",
           "clip": "clip_cos", "lpips": "lpips_neg"}.get(variant.split("_")[1])
...
def main():
    ...
    ap.add_argument("--coupling", default="fresh",
                    choices=["fresh", "teacher", "perm_within", "perm_global",
                             "fixed_unrelated"],
    ...
    if is_main and not args.no_wandb:
        ...
        wandb.init(project=args.wandb_project, name=args.wandb_run_name, config=cfg_all)
    ...
    while gstep < args.num_steps:
        ...
        if is_consistency:
            ...
            deltas = {kt: random.randint(args.delta_min, args.delta_max) for kt in score_idxs}
