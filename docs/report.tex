...
\label{eq:unified}
\end{equation}
where the arms differ in four things and nothing else:
\begin{enumerate}
\item \textbf{The weights} $w(c)$ over the candidates. They are a function of a per-candidate score $S_j(c)$ and a temperature: one-hot on $\argmax_jS_j$ (argmax), one-hot on a stored uniform draw (random), the Boltzmann weights $\pi_j=\exp(S_j/T)/\sum_l\exp(S_l/T)$, or uniform $1/N$.
\item \textbf{The estimator} of the weighted sum. \emph{Exact}: every candidate with $w_j>0$ is rolled out and its loss is weighted by $w_j$ (cost proportional to the number of candidates). \emph{Sampled}: one index $J\sim w$ is drawn on each visit of the caption and $\ell^{(J)}$ is used (single-candidate cost; same expected gradient, different optimizer updates, Section~\ref{sec:graddiag}). \emph{Kept}: $J_c\sim w$ is drawn once per caption and reused on every visit. \emph{Same-caption Monte Carlo}: $M$ independent draws per visit, losses weighted by their counts over $M$ (implemented, not run). For one-hot weights the four estimators coincide.
\item \textbf{The score} $S_j$. RGB: $S_j=\langle u^{\mathrm{pat}}(\Dec(z_K^{(j)})),u^{\mathrm{pat}}(x^{\mathrm{ref}})\rangle$, the DINOv2 patch-mean cosine between the decoded candidate and the reference photograph (Section~\ref{sec:scorers}). Latent: $\hat S_j=\langle P(z_K^{(j)}),u^{\mathrm{pat}}(x^{\mathrm{ref}})\rangle$, with the candidate side replaced by a projector on the terminal latent (Section~\ref{sec:latent}). VQAScore and the other scorers of Table~\ref{tab:headroom} are further choices of $S$.
\item \textbf{The reward} $r_c(\theta)=\frac{1}{R}\sum_{s\in\mathcal{R}}\langle P(\hat{x}_0(z^{(J)}_s;\theta)),u^{\mathrm{pat}}(x^{\mathrm{ref}})\rangle$ on the student's own clean estimates from the $R$ least-noisy supervised inputs, with $\lambda>0$ only in the reward arms of Section~\ref{sec:reward}; $P$ is frozen or refreshed against the RGB scorer during training. Everywhere else $\lambda=0$.
\end{enumerate}
