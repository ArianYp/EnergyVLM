...
\item \textbf{Weight from a gradient probe.} On eight updates at $\lambda=1$ the reward gradient has median norm 7.1 against 497 for the consistency gradient (ratio 0.013), with cosine 0.04 between the two, so the reward pulls in a direction the consistency loss is indifferent to. $\lambda=15.5$ puts the reward at the same 20\% of the consistency gradient that the projector arms used.
...
\caption{The exact reward against the projector rewards, in training. (a) The offline RGB DINO score of the decoded predictions (the hacking monitor, 32 latents every 100 updates, 700-update running mean; bold is the seed mean). (b) The reward seen by the optimizer minus that monitor: for the exact reward the two are the same quantity up to the resize path (correlation 0.9997); for the projector arms the gap is $-0.19$ and the two move apart. (c) The consistency loss of the exact-reward arm against argmax without a reward, 200-update means.}
\label{fig:rewardx}
\end{figure}

\paragraph{In training.}
Figure~\ref{fig:rewardx} shows what the projector arms could not do. The true RGB score of the predictions rises: from 0.583 over updates 100 to 300 to 0.608 over the last 200 updates, per seed $+0.018$, $+0.050$ and $+0.006$, where the projector arms fell by 0.014 (Table~\ref{tab:reward}). The reward the optimizer sees and the monitor agree throughout (correlation 0.9997 over every monitor batch), so there is no proxy to hack; and the consistency loss is unchanged (70.5 against 69.6 for argmax over the last 1,000 updates), so the reward is not paid for with the distillation objective. The reward is optimised for real. What that buys on held-out captions and on the benchmarks is the question.
