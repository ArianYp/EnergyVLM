...
\label{alg:reward}
\begin{algorithmic}[1]
\Require projector $P$ (frozen weights, differentiable in its input); $u_{\mathrm{ref}}$ per caption; $\lambda=80$; buffer $B$ of recent $\hat{x}_0$; RGB scorer; replay set of 2,000 teacher-candidate captions
\Procedure{Reward}{$\hat{x}_0^{(s_1)},\hat{x}_0^{(s_2)}$} \Comment{the two clean estimates from the least-noisy inputs}
  \State push $\hat{x}_0^{(s_1)}$ (detached) into $B$;\quad \Return $\frac12\sum_{i}\langle P(\hat{x}_0^{(s_i)}),u_{\mathrm{ref}}\rangle$ \Comment{gradient flows through $P$ into $\theta$}
\EndProcedure
