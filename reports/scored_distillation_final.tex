...
\begin{table}[ht]
\centering
\caption{Headline results. Alignment deltas are against the random-selection student. The 3k rows use three training seeds; intervals are 95\% hierarchical bootstrap over seeds and prompts. The averaged 3k rows (Section~\ref{sec:selrule} protocol: average of the 2k/4k/6k checkpoints) give seed-paired mean $\pm$ sd over seeds with the seed $t$-test $p$. The 118k rows use three training seeds and weight-averaged checkpoints, with per-prompt paired tests pooled over seeds; the VQAScore arm at 118k is one seed. CMMD is the seed mean.}
\label{tab:summary}
\fitbox{%
...
\midrule
3k, averaged checkpoints, 3 seeds & random (baseline) & 0.4738 & 22.53 & 0.84 \\
3k, averaged checkpoints, 3 seeds & DINOv2 patches & $+0.0135 \pm 0.0042$ ($p=0.03$) & $+1.19 \pm 1.66$ & 0.80 \\
3k, averaged checkpoints, 3 seeds & DINOv2 patches + exact DINO reward & $+0.0246 \pm 0.0049$ ($p=0.013$) & $+2.71 \pm 0.87$ ($p=0.03$) & 0.69 \\
\midrule
...
The factorial is running at the time of writing, with the following predeclared design.
\begin{itemize}
\item \textbf{Cells and seeds.} Five training seeds per cell. The two reward-free cells keep their seeds 0 to 2 and add seeds 3 and 4 under the same frozen trainer; the two reward cells are trained fresh for seeds 0 to 4 under the RNG-restored trainer below, so the first exact-reward runs of Section~\ref{sec:rewardx} become a replication under a different caption order rather than a cell of the design.
...
\item \textbf{The exact reward is a 3k-pool result.} Three seeds, one $\lambda$, one schedule, and a caption order that differs from the argmax arm's at the same seed (Section~\ref{sec:rewardx}): its CompBench gain over argmax is $+0.011\pm0.006$ ($p=0.085$ paired, $0.02$ unpaired), its fidelity gain is on every seed, and neither has been tested at 118k, with the stronger teachers, or with a swept $\lambda$. It also uses the reference photograph a second time, so it inherits the scorer's need for a real image per caption.
