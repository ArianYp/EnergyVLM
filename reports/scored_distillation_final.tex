...
\subsection{The exact reward at 118k captions}
\label{sec:reward118k}
The exact reward (Section~\ref{sec:rewardx}) was re-tested at the full 118k scale, three seeds, otherwise identical to the argmax-only 118k configuration (Table~\ref{tab:hyper}), $\lambda=15.504$. Table~\ref{tab:reward118k} gives the result. Over argmax alone the reward adds $+0.0087\pm0.0047$ CompBench (every seed positive, $p=0.08$) and $+1.45\pm0.80$ GenEval2; over random selection the combined effect is $+0.0262\pm0.0025$ CompBench ($p=0.003$) and $+2.74$ GenEval2. Fidelity moves further in the same direction selection alone already moved it: CMMD falls from 0.783 (argmax) to 0.683, precision rises from 0.518 to 0.563 and recall from 0.067 to 0.087, on every seed, with FID unchanged (30.79 against 31.09). The 3k-scale result (Section~\ref{sec:rewardx}: $+0.0055$ CompBench over argmax, CMMD $0.79\to0.73$) holds up at 40 times the caption count and roughly 10 times the update count: the reward's benefit is not an artefact of the small pool or the short schedule.

\begin{table}[ht]
\centering
\caption{The exact reward at 118k captions, 56,974 updates, three training seeds, weight-averaged students. Fidelity: 5,000 COCO \texttt{val2017} captions, seed means.}
\label{tab:reward118k}
\fitbox{%
\begin{tabular}{l c c c c c c}
\toprule
Arm & CompBench & $\Delta$ vs.\ argmax & GenEval2 & CMMD $\downarrow$ & Precision & Recall \\
\midrule
Random selection & 0.4668 & -- & 20.29 & 0.837 & 0.492 & 0.062 \\
Argmax, no reward & 0.4843 & -- & 21.58 & 0.783 & 0.518 & 0.067 \\
Argmax + exact reward & 0.4930 $\pm$ 0.0033 & $+0.0087\pm0.0047$ ($p{=}0.08$) & 23.03 $\pm$ 1.15 & 0.683 & 0.563 & 0.087 \\
\bottomrule
\end{tabular}}
\end{table}
